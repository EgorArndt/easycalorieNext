...
import React, { useMemo, forwardRef, ComponentType, CSSProperties, NamedExoticComponent } from 'react'
...
export type WithStylesProps = {
    ...
    color?: keyof PaletteProps | keyof AppTheme['readonly']['commonColors'] | false
    border?: boolean 
    borderTop?: boolean 
    borderBottom?: boolean 
    borderLeft?: boolean 
    borderRight?: boolean 
    borrowPaletteFrom?: keyof AppTheme['mutatable']['ui'] | null
    ...
    center?: boolean
    align?: 
        [CSSProperties['justifyContent'] | null, CSSProperties['alignItems'] | null] 
        | 
...
export const withStyles = <P extends WithStylesProps>(
    ...
    const StyledComponent = forwardRef(({ 
        ...
        const { 
            ...
            bgOnHover, 
            textOnHover 
        } = usePalette(palette, borrowPaletteFrom || initialKey)
        ...
        const styles = useMemo(() => {
            return css`
                ${(height || fullSize) && ('height:' + (fullSize ? '100%' : cssUnitByType(height)))};
                ${(width || fullSize) && ('width:' + (fullSize ? '100%' : cssUnitByType(width)))};
                ...
                ${column && (align || center) ? `
                    ${typeof align === 'string' ? 
                    (align === 'left' ? 'align-items: start' : align === 'right' ? 'align-items: end' : `justify-content: ${align}`) + ' !important;'
                    : `justify-content: ${(center ? 'center' : align && align[0]) + ' !important;'} align-items: ${(center ? 'center' : align && align[1]) + ' !important'};`}
                `
                : (align || center) ? `
                    ${typeof align === 'string' ? `justify-content: ${(center ? 'center' : align) + ' !important'};` : `justify-content: ${(center ? 'center' : align && align[0]) + ' !important;'} align-items: ${(center ? 'center' : align && align[1]) + ' !important'};`}
                `: null}
                ${bg && `background-color: ${bg} ${important};`}
                ${(color || contrastText) && `color: ${color ? textColors[color] || commonColors[color].bg : contrastText} ${important};`}
            ...
            `
        }, [fullSize, height, width, gap, wrap, noWrap, column, center, align, bg, bgOnHover, border, childrenSpacing, color, contrastText, fontSize, spacing, textOnHover, textColors, _border, borderTop, borderBottom, borderLeft, borderRight, inherits_palette, active])
