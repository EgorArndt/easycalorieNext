...
export const StyledCore = styled.button<Partial<ButtonBaseProps> & Additional>`&&&{
    ...
    ${({ _onActive, variant }) => _onActive && css`
        background-color: ${_onActive.bg} !important;
        color: ${variant && ['bgless', 'default'].includes(variant) ? _onActive.bg : _onActive.contrastText} !important;
    ...
    &.btn-contained-reversed {
        ${({ derivedStyles }) => derivedStyles && css`
